...
export default function MinimalInvoiceDocument({
  document: doc,
  language = 'ku',
  format = 'a4',
  className = '',
  id = 'printable-invoice-master'
}: MinimalInvoiceDocumentProps) {
  ...
  const getStatusBadge = () => {
    ...
    if (s === 'paid') {
      return {
        ...
        bg: 'bg-gray-50 text-emerald-800 border-emerald-300'
      };
    }
    if (s === 'overdue') {
      return {
        label: isKu ? 'دواکەوتوو (Overdue)' : 'Overdue',
        bg: 'bg-gray-50 text-rose-800 border-rose-300'
      };
    }
    if (s === 'partially_paid') {
      return {
    ...
    if (s === 'installment') {
      return {
        ...
        bg: 'bg-indigo-50 text-black border-indigo-300'
      };
    }
    if (s === 'debt') {
      return {
    ...
    return {
      ...
      bg: 'bg-gray-100 text-black border-gray-200'
    };
  };
  ...
  if (isThermal) {
    return (
      <div 
        id={id}
        dir={isRtl ? 'rtl' : 'ltr'}
        className={`w-full max-w-[360px] mx-auto bg-white text-black font-sans p-4 shadow-md text-xs leading-relaxed print:p-0 print:shadow-none print:max-w-none print:w-full ${className}`}
      >
        {/* Thermal Header */}
        <div className="text-center pb-3 border-b border-dashed border-slate-400">
          ...
          <p className="text-[10px] text-gray-700 mt-0.5">{storeTagline}</p>
          <p className="text-[10px] text-gray-500 font-mono mt-0.5" dir="ltr">{storePhone}</p>
        ...
        {/* Thermal Meta */}
        <div className="py-2 border-b border-dashed border-gray-200 space-y-1 text-[11px]">
          <div className="flex justify-between items-center">
            <span className="text-gray-500">{isKu ? 'ژمارە:' : 'Doc No:'}</span>
            <span className="font-mono font-bold text-black" dir="ltr">{doc.documentNumber}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-gray-500">{isKu ? 'بەروار:' : 'Date:'}</span>
            <span className="font-mono text-black" dir="ltr">{doc.issueDate} {doc.issueTime || ''}</span>
          </div>
          {doc.customer?.name && (
            <div className="flex justify-between items-center">
              <span className="text-gray-500">{isKu ? 'کڕیار:' : 'Customer:'}</span>
              <span className="font-bold text-black">{doc.customer.name}</span>
            </div>
          )}
          {doc.customer?.phone && (
            <div className="flex justify-between items-center">
              <span className="text-gray-500">{isKu ? 'مۆبایل:' : 'Phone:'}</span>
              <span className="font-mono text-black" dir="ltr">{doc.customer.phone}</span>
            </div>
          )}
        </div>
        ...
        {Array.isArray(doc.items) && doc.items.length > 0 && (
          <div className="py-2 border-b border-dashed border-gray-200">
            <div className="flex justify-between text-[10px] font-bold text-gray-500 uppercase pb-1 mb-1 border-b border-gray-200">
            ...
            <div className="space-y-1.5">
              {(doc.items || []).map((item, idx) => (
                <div key={item.id || idx} className="text-[11px]">
                  ...
                  {item.imei && (
                    <div className="text-[10px] text-gray-700 font-mono tracking-wider" dir="ltr">
                      IMEI: {item.imei}
                    </div>
                  )}
        ...
        <div className="py-2 border-b border-dashed border-slate-400 space-y-1.5 text-xs">
          <div className="flex justify-between text-slate-700">
            ...
            <span className="font-mono font-bold text-black" dir="ltr">{formatCurrency(doc.grandTotal, cur)}</span>
          </div>
          ...
          {doc.remainingBalance > 0 && (
            <div className="flex justify-between text-rose-800 font-black text-sm pt-1 border-t border-gray-200">
        ...
        {doc.installmentDetails && (
          <div className="py-2 border-b border-dashed border-gray-200 text-[10px] space-y-0.5">
            <div className="font-bold text-black">{isKu ? 'پوختەی قیست:' : 'Installment Plan:'}</div>
        ...
        <div className="pt-3 text-center space-y-2">
          ...
          <p className="text-[10px] text-gray-700 leading-tight">{footerMessage}</p>
  ...
  return (
    <div 
      id={id}
      dir={isRtl ? 'rtl' : 'ltr'}
      className={`w-full max-w-[840px] mx-auto bg-white text-black font-sans p-8 sm:p-10 md:p-12 shadow-2xl rounded-xl border border-gray-200 text-xs leading-normal print:p-0 print:shadow-none print:border-none print:rounded-none print:max-w-none print:w-full ${className}`}
    >
      {/* 1. TOP HEADER & BRANDING */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 pb-6 border-b border-black">
        
        {/* Business Info Left/Right depending on RTL */}
        <div className="flex items-center gap-4">
          {storeLogo ? (
            <div className="w-16 h-16 rounded-xl border border-gray-200 bg-slate-50 flex items-center justify-center p-1 overflow-hidden shrink-0">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-700 font-medium mt-0.5">{storeTagline}</p>
        ...
        <div className={`flex flex-col sm:items-end ${isRtl ? 'sm:text-left' : 'sm:text-right'} w-full sm:w-auto`}>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {doc.paymentMethod && (
              <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-gray-100 text-slate-700 border border-gray-200">
                {doc.paymentMethod}
              </span>
            )}
          </div>
          <div className="text-[11px] font-mono text-gray-500 mt-1" dir="ltr">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-5 border-b border-gray-200">
        
        {/* Document Identification */}
        <div className="bg-white p-2.5 rounded-xl border border-gray-200/80 space-y-1.5">
          ...
          <div className="flex justify-between items-center text-xs">
            <span className="text-gray-500">{isKu ? 'ژمارەی پسوڵە:' : 'Invoice No:'}</span>
            <span className="font-mono font-bold text-black" dir="ltr">{doc.documentNumber}</span>
          </div>
          <div className="flex justify-between items-center text-xs">
            <span className="text-gray-500">{isKu ? 'بەروار:' : 'Issue Date:'}</span>
            <span className="font-mono font-medium text-black" dir="ltr">{doc.issueDate}</span>
          </div>
          {doc.issueTime && (
            <div className="flex justify-between items-center text-xs">
              <span className="text-gray-500">{isKu ? 'کاتژمێر:' : 'Time:'}</span>
              <span className="font-mono text-slate-700" dir="ltr">{doc.issueTime}</span>
            </div>
          )}
          {doc.sellerName && (
            <div className="flex justify-between items-center text-xs">
              <span className="text-gray-500">{isKu ? 'فرۆشیار:' : 'Cashier:'}</span>
              <span className="font-medium text-black">{doc.sellerName}</span>
            </div>
          )}
        </div>

        {/* Customer Information */}
        <div className="bg-white p-2.5 rounded-xl border border-gray-200/80 space-y-1.5 md:col-span-2">
          <div className="text-[10px] font-bold uppercase tracking-wider text-gray-500 flex items-center justify-between">
            ...
            {doc.customer?.idCard && (
              <span className="font-mono text-[10px] text-gray-700" dir="ltr">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
            <div className="flex justify-between items-center">
              <span className="text-gray-500">{isKu ? 'ناوی کڕیار:' : 'Name:'}</span>
              <span className="font-bold text-black">{doc.customer?.name || (isKu ? 'کڕیاری گشتی' : 'Walk-in Customer')}</span>
            </div>

            {doc.customer?.phone && (
              <div className="flex justify-between items-center">
                <span className="text-gray-500">{isKu ? 'ژمارەی مۆبایل:' : 'Phone:'}</span>
                <span className="font-mono font-bold text-black" dir="ltr">{doc.customer.phone}</span>
              </div>
            )}

            {doc.customer?.address && (
              <div className="flex justify-between items-center sm:col-span-2">
                <span className="text-gray-500">{isKu ? 'ناونیشان:' : 'Address:'}</span>
                <span className="text-black">{doc.customer.address}</span>
              </div>
            )}

            {doc.customer?.guarantorName && (
              <div className="flex justify-between items-center sm:col-span-2 pt-1 border-t border-gray-200/60 text-[11px]">
                <span className="text-gray-500">{isKu ? 'کەفیل (زامن):' : 'Guarantor:'}</span>
        ...
        <div className="py-5 border-b border-gray-200">
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full text-xs text-left border-collapse">
              <thead>
                <tr className="bg-gray-100 text-slate-700 font-bold border-b border-gray-200 text-[11px]">
                ...
                {doc.items.map((item, idx) => {
                  ...
                  return (
                    <tr key={item.id || idx} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="py-2.5 px-3">
                        {item.imei ? (
                          <div className="inline-block px-2 py-0.5 bg-gray-100 rounded border border-gray-200 font-mono text-[10px] font-bold text-black tracking-wider" dir="ltr">
                            IMEI: {item.imei}
                          </div>
                        ) : item.barcode ? (
                          <div className="font-mono text-[10px] text-gray-700" dir="ltr">
                            BAR: {item.barcode}
                          </div>
                        ) : (
                      ...
                      <td className="py-2.5 px-3 text-right font-mono text-black text-[11px]" dir="ltr">
      ...
      {doc.installmentDetails && (
        <div className="py-5 border-b border-black">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            ...
            <div className="p-3 bg-gray-50 border border-emerald-300 text-emerald-950 rounded-xl">
            ...
            <div className="p-3 bg-indigo-50 border border-indigo-300 text-indigo-950 rounded-xl">
              ...
              <div className="font-mono text-base font-black text-black mt-1" dir="ltr">
          ...
          {doc.installmentDetails.totalAmount > 0 && (() => {
            ...
            return (
              <div className="mb-5 bg-slate-50 p-4 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
                ...
                <div className="flex justify-between items-end mb-3 relative z-10">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-lg font-black text-black font-mono tracking-tight" dir="ltr">
                ...
                <div className="flex justify-between mt-2 text-[10px] font-bold text-gray-500 font-mono uppercase tracking-wider relative z-10">
                  <span className="flex items-center gap-1">
          ...
          <div className="text-[11px] font-bold uppercase tracking-wider text-black mb-3 flex items-center justify-between">
            <span className="flex items-center gap-1.5">
            ...
            <table className="w-full text-xs text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-200">
                {doc.installmentDetails.schedules.map((s, idx) => {
                  ...
                  return (
                    <tr key={idx} className={isPaid ? 'bg-gray-50/50' : isOverdue ? 'bg-gray-50/60 font-semibold' : 'hover:bg-slate-50'}>
                      <td className="py-2 px-3 text-center font-bold text-black">
                        {s.monthNumber}
                      </td>
                      <td className="py-2 px-3 font-mono text-black font-medium" dir="ltr">
                        {s.dueDate}
                      </td>
                      ...
                      <td className="py-2 px-3 text-[10px] font-mono text-gray-700">
      ...
      {doc.statementDetails && (
        <div className="py-5 border-b border-gray-200">
          <div className="text-[11px] font-bold uppercase tracking-wider text-black mb-2 flex items-center justify-between">
            <span className="flex items-center gap-1.5">
              ...
              <tbody className="divide-y divide-slate-100">
                {doc.statementDetails.transactions.map((tx, idx) => (
                  <tr key={tx.id || idx} className="hover:bg-slate-50/70">
                    ...
                    <td className="py-2 px-3 font-mono font-bold text-black" dir="ltr">{tx.referenceNo}</td>
                    <td className="py-2 px-3 text-black">{tx.description}</td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-6 border-b border-black">
        ...
        <div className="flex flex-col justify-between space-y-3 bg-slate-50 p-5 rounded-2xl border border-gray-200">
          
          <div className="space-y-2 text-xs">
            {/* Subtotal */}
            <div className="flex justify-between items-center text-gray-700">
              <span>{isKu ? 'کۆی سەرەتایی (بێ داشکاندن):' : 'Subtotal:'}</span>
              <span className="font-mono font-medium text-black" dir="ltr">{formatCurrency(doc.subtotal, cur)}</span>
            </div>
            ...
            {doc.tax > 0 && (
              <div className="flex justify-between items-center text-gray-700">
                ...
                <span className="font-mono text-black" dir="ltr">+{formatCurrency(doc.tax, cur)}</span>
              </div>
            )}

            {/* Grand Total */}
            <div className="flex justify-between items-center pt-2 border-t border-gray-200 text-sm font-bold text-slate-950">
          ...
          <div className={`p-4 rounded-xl border-2 transition-all ${
            doc.remainingBalance > 0 
              ? 'bg-gray-50 border-rose-400 text-rose-950' 
              : 'bg-gray-50 border-emerald-400 text-emerald-950'
          }`}>
            <div className="flex justify-between items-center">
              <div>
      ...
      {(doc.documentType === 'installment_invoice' || doc.documentType === 'debt_invoice') && (
        <div className="py-4 border-b border-gray-200 bg-slate-50/80 p-4 rounded-xl border">
          <div className="flex items-center gap-2 mb-2 text-black font-black text-xs uppercase tracking-wider">
      ...
      <div className="pt-6 space-y-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center text-xs">
          ...
          {/* Customer / Debtor */}
          <div className="p-3 bg-slate-50 rounded-xl border border-gray-200 flex flex-col justify-between min-h-[140px]">
            <div>
            ...
            <div className="w-10 h-12 border-2 border-gray-200 rounded mx-auto flex items-center justify-center text-[9px] text-gray-400 font-mono">
              {isKu ? 'جێ پەنجە' : 'Thumb'}
            </div>
          </div>
        ...
        {/* Footer Text */}
        <div className="text-center pt-4 border-t border-gray-200 text-gray-500 space-y-1">
