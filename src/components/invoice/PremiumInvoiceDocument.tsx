...
interface PremiumInvoiceDocumentProps {
...
export default function PremiumInvoiceDocument({
  document: doc,
  language = 'ku',
  format = 'a4',
  className = '',
  id = 'printable-invoice-master'
}: PremiumInvoiceDocumentProps) {
    ...
    return (
      ...
      >
        ...
        <div className="text-center pb-3 border-b border-dashed border-slate-400">
          ...
          <p className="text-[10px] text-slate-600 mt-0.5">{storeTagline}</p>
          <p className="text-[10px] text-slate-500 font-mono mt-0.5" dir="ltr">{storePhone}</p>
        ...
        <div className="py-2 border-b border-dashed border-slate-300 space-y-1 text-[11px]">
          <div className="flex justify-between items-center">
            <span className="text-slate-500">{isKu ? 'ژمارە:' : 'Doc No:'}</span>
            <span className="font-mono font-bold text-slate-900" dir="ltr">{doc.documentNumber}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-slate-500">{isKu ? 'بەروار:' : 'Date:'}</span>
            <span className="font-mono text-slate-800" dir="ltr">{doc.issueDate} {doc.issueTime || ''}</span>
          </div>
          {doc.customer?.name && (
            <div className="flex justify-between items-center">
              <span className="text-slate-500">{isKu ? 'کڕیار:' : 'Customer:'}</span>
              <span className="font-bold text-slate-900">{doc.customer.name}</span>
            </div>
          )}
          {doc.customer?.phone && (
            <div className="flex justify-between items-center">
              <span className="text-slate-500">{isKu ? 'مۆبایل:' : 'Phone:'}</span>
              <span className="font-mono text-slate-800" dir="ltr">{doc.customer.phone}</span>
            </div>
          )}
        </div>
        ...
        {Array.isArray(doc.items) && doc.items.length > 0 && (
          <div className="py-2 border-b border-dashed border-slate-300">
            <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase pb-1 mb-1 border-b border-slate-200">
            ...
            <div className="space-y-1.5">
              {(doc.items || []).map((item, idx) => (
                <div key={item.id || idx} className="text-[11px]">
                  ...
                  {item.imei && (
                    <div className="text-[10px] text-slate-600 font-mono tracking-wider" dir="ltr">
                      IMEI: {item.imei}
                    </div>
                  )}
        ...
        <div className="py-2 border-b border-dashed border-slate-400 space-y-1.5 text-xs">
          ...
          {doc.remainingBalance > 0 && (
            <div className="flex justify-between text-rose-800 font-black text-sm pt-1 border-t border-slate-200">
        ...
        <div className="pt-3 text-center space-y-2">
          ...
          <p className="text-[10px] text-slate-600 leading-tight">{footerMessage}</p>
  ...
  return (
    <div 
      id={id}
      dir={isRtl ? 'rtl' : 'ltr'}
      className={`w-full max-w-[840px] mx-auto bg-white text-slate-900 font-sans p-8 sm:p-10 md:p-12 shadow-2xl rounded-xl border border-slate-200 text-xs leading-normal print:p-0 print:shadow-none print:border-none print:rounded-none print:max-w-none print:w-full ${className}`}
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 pb-6 border-b-2 border-slate-900">
        ...
        <div className="flex items-center gap-4">
          {storeLogo ? (
            <div className="w-16 h-16 rounded-xl border border-slate-200 bg-slate-50 flex items-center justify-center p-1 overflow-hidden shrink-0">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-600 font-medium mt-0.5">{storeTagline}</p>
        ...
        <div className={`flex flex-col sm:items-end ${isRtl ? 'sm:text-left' : 'sm:text-right'} w-full sm:w-auto`}>
          ...
          <div className="text-[11px] font-mono text-slate-500 mt-1" dir="ltr">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-5 border-b border-slate-200">
        ...
        <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200/80 space-y-1.5">
          ...
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-500">{isKu ? 'ژمارەی پسوڵە:' : 'Invoice No:'}</span>
            <span className="font-mono font-bold text-slate-900" dir="ltr">{doc.documentNumber}</span>
          </div>
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-500">{isKu ? 'بەروار:' : 'Issue Date:'}</span>
            <span className="font-mono font-medium text-slate-800" dir="ltr">{doc.issueDate}</span>
          </div>
          {doc.issueTime && (
            <div className="flex justify-between items-center text-xs">
              <span className="text-slate-500">{isKu ? 'کاتژمێر:' : 'Time:'}</span>
              <span className="font-mono text-slate-700" dir="ltr">{doc.issueTime}</span>
            </div>
          )}
          {doc.sellerName && (
            <div className="flex justify-between items-center text-xs">
              <span className="text-slate-500">{isKu ? 'فرۆشیار:' : 'Cashier:'}</span>
              <span className="font-medium text-slate-800">{doc.sellerName}</span>
            </div>
          )}
        </div>

        {/* Customer Information */}
        <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200/80 space-y-1.5 md:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
            <div className="flex justify-between items-center">
              <span className="text-slate-500">{isKu ? 'ناوی کڕیار:' : 'Name:'}</span>
              <span className="font-bold text-slate-900">{doc.customer?.name || (isKu ? 'کڕیاری گشتی' : 'Walk-in Customer')}</span>
            </div>

            {doc.customer?.phone && (
              <div className="flex justify-between items-center">
                <span className="text-slate-500">{isKu ? 'ژمارەی مۆبایل:' : 'Phone:'}</span>
                <span className="font-mono font-bold text-slate-800" dir="ltr">{doc.customer.phone}</span>
              </div>
            )}

            {doc.customer?.address && (
              <div className="flex justify-between items-center sm:col-span-2">
                <span className="text-slate-500">{isKu ? 'ناونیشان:' : 'Address:'}</span>
                <span className="text-slate-800">{doc.customer.address}</span>
              </div>
            )}

            {doc.customer?.guarantorName && (
              <div className="flex justify-between items-center sm:col-span-2 pt-1 border-t border-slate-200/60 text-[11px]">
                <span className="text-slate-500">{isKu ? 'کەفیل (زامن):' : 'Guarantor:'}</span>
                ...
                {doc.items.map((item, idx) => {
                  ...
                  return (
                    <tr key={item.id || idx} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="py-2.5 px-3">
                        {item.imei ? (
                          <div className="inline-block px-2 py-0.5 bg-slate-100 rounded border border-slate-300 font-mono text-[10px] font-bold text-slate-900 tracking-wider" dir="ltr">
                            IMEI: {item.imei}
                          </div>
                        ) : item.barcode ? (
      ...
      {doc.installmentDetails && (
        <div className="py-5 border-b-2 border-slate-900">
          ...
          {doc.installmentDetails.totalAmount > 0 && (() => {
            ...
            return (
              <div className="mb-5 bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
                ...
                <div className="flex justify-between mt-2 text-[10px] font-bold text-slate-500 font-mono uppercase tracking-wider relative z-10">
                  <span className="flex items-center gap-1">
                ...
                {doc.installmentDetails.schedules.map((s, idx) => {
                  ...
                  return (
                    <tr key={idx} className={isPaid ? 'bg-emerald-50/50' : isOverdue ? 'bg-rose-50/60 font-semibold' : 'hover:bg-slate-50'}>
                      ...
                      <td className="py-2 px-3 font-mono text-slate-800 font-medium" dir="ltr">
                        {s.dueDate}
                      </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-6 border-b-2 border-slate-900">
        ...
        <div className="flex flex-col justify-between space-y-3 bg-slate-50 p-5 rounded-2xl border border-slate-200">
          ...
          <div className="space-y-2 text-xs">
            ...
            {/* Grand Total */}
            <div className="flex justify-between items-center pt-2 border-t border-slate-200 text-sm font-bold text-slate-950">
      ...
      {(doc.documentType === 'installment_invoice' || doc.documentType === 'debt_invoice') && (
        <div className="py-4 border-b border-slate-300 bg-slate-50/80 p-4 rounded-xl border">
      ...
      <div className="pt-6 space-y-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center text-xs">
          ...
          {/* Customer / Debtor */}
          <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex flex-col justify-between min-h-[140px]">
            <div>
            ...
            <div className="w-10 h-12 border-2 border-slate-300 rounded mx-auto flex items-center justify-center text-[9px] text-slate-400 font-mono">
              {isKu ? 'جێ پەنجە' : 'Thumb'}
            </div>
          </div>
        ...
        {/* Footer Text */}
        <div className="text-center pt-4 border-t border-slate-200 text-slate-500 space-y-1">
